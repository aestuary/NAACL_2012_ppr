...
Frustratingly Easy Domain Adaptation, Hal Daume III, 2007
			? fully supervised setting for learning a function to map input space X to 
    decision/output space Y that optimizes itÕs performance on target    
    (in-domain) data
...
Previous similarity score based on monolingual distributional similarity was computed using the Google ngram corpus \cite{LinEtAlLREC10}, which comprises of statistics from general domain of resources from the internet. Although more data leads to a better vector space model for the score, reranking of the paraphrases can potentially benefitted from adapting the training corpus to the target domain. ttttttt
...
\subsection{Data and Language Model Parameters}
\label{sect:data_LM_params}
